import { Command, Context, EinClient } from '../../core/index.js';
    ...
    public async run(client: EinClient, ctx: Context, args: string[]): Promise<void> {
        ...
        if (!args[0]) {
            ...
            const helpEmbed = embed
                ...
                .setTitle('Help Menu')
                .setDescription(
                    `Hey there! I'm ${this.client.user.username}, a music bot. You can use \`${prefix}help <command>\` to get more info on a command.`,
                )
                .setFooter({
